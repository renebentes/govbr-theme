    ...
    <?php foreach ($this->children[$this->category->id] as $id => $child) : ?>
        ...
        <?php if (\in_array($child->access, $groups)) : ?>
            <?php if ($this->params->get('show_empty_categories') || $child->getNumItems(true) || \count($child->getChildren())) : ?>
                <div class="br-item" role="listitem" <?= (\count($child->getChildren()) > 0 && $this->maxLevel > 1) ? ' data-toggle="collapse" data-target="category-' . $child->id . '"' : ''; ?>>
                    <div class="row">
                        <div class="col">
                            <h4 class="my-1">
                                <?php if ($lang->isRtl()) : ?>
                                    <?php if ($this->params->get('show_cat_num_articles', 1)) : ?>
                                        <span class="br-tag count bg-info" data-tooltip-text="<?= HTMLHelper::_('tooltipText', 'COM_CONTENT_NUM_ITEMS'); ?>">
                                            <?= $child->getNumItems(true); ?>
                                        </span>
                                    <?php endif; ?>
                                    <a href="<?= Route::_(RouteHelper::getCategoryRoute($child->id, $child->language)); ?>">
                        ...
                        <?php if (\count($child->getChildren()) > 0 && $this->maxLevel > 1) : ?>
                            <div class="col-auto">
                                <button type="button" class="br-button circle small" aria-label="<?= Text::_('JGLOBAL_EXPAND_CATEGORIES'); ?>">
